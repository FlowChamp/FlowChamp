...
class Header extends Component {
   constructor(props) {
      super(props);
      const { year, quarter, blockIndex, flowchart } = props;
      this.state = {
         course: flowchart.chartData[year].quarters[quarter][blockIndex],
         shouldClose: false,
      };
   }

   static getDerivedStateFromProps(nextProps, prevState) {
      const { year, quarter, blockIndex, flowchart } = nextProps;
      const nextCourse =
         flowchart.chartData[year].quarters[quarter][blockIndex];
      const shouldClose =
         !nextCourse ||
         nextCourse.block_metadata._id !== prevState.course.block_metadata._id;
